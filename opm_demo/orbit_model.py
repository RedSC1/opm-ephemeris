...
class HelioProvider:
    ...
    def velocity(self, jd_arr: np.ndarray) -> np.ndarray:
        ...
        for target_seg, sun_seg in zip(self.target_segments, self.sun_segments):
            ...
            if not np.any(mask):
                continue
            _xt, vt = target_seg.compute_and_differentiate(tdb[mask])
            _xs, vs = sun_seg.compute_and_differentiate(tdb[mask])
            out[mask, 0] = vt[0] - vs[0]
            out[mask, 1] = vt[1] - vs[1]
            out[mask, 2] = vt[2] - vs[2]
        return out
