...
月球是当前体积的主要来源，约占 600 年文件总量的 70%；水星、太阳、金星和地月质心次之。外行星由于运动较慢、分段数少，单体文件在 600 年范围内只有数 KiB 到十余 KiB。作为同一时间分片的文件体积参照，本地 DE441 版本 Swiss Ephemeris 中相对应的核心文件为 `sepl_18.se1`（太阳至冥王星）和 `semo_18.se1`（月球），二者合计 `1,788,832` 字节，即约 `1.71 MiB`；这里不计入小行星文件 `seas_18.se1`。若按表 6 的总每世纪体积线性外推到完整 DE441 时间跨度 JD -3100015--8000016（约 303.9 个儒略世纪），全套主要天体数据约为 82.9 MiB。该外推只用于量级比较，未经完整 DE441 范围生产验证。
